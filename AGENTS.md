...
`components/canvas/scenes/HeroScene.tsx` + `shaders/wave.ts`. A calm band of horizontal threads (one `LineSegments` draw call, thread/segment counts per GPU tier in `loomDensity`). All motion lives in the vertex shader, deterministic from `(uTime, row, x)`: the left side ripples with slow turbulence (the unformed idea) and eases into perfectly straight, faintly teal lines on the right (the delivered product) — chaos in → order out, no particles. A faint brightening at x = 0 marks the brand seam. Pointer raises a gentle swell (unprojected to the z = 0 plane, then loom-local); scroll tips the band and fades it out near the section end; `uTime` stops advancing once well past the hero. Portrait lifts the band above the bottom-anchored copy via `lib/canvas-clear.ts`. All tuning in `design-data.json → hero` (`bandHeight`, `ampChaos/ampOrder`, `speed`, `tilt`, `seamBoost`…). The loader's `hero` milestone fires on the first rendered frame (geometry is synchronous).

## Section transitions

Two connective devices, deliberately split: the composite wipe's **teal seam glow** rides only the showcase transitions home→services and services→references (gated in `World.tsx` by current section id; intensity/width in `design-data.json → composite.seamGlow/seamWidth`). Every other boundary is stitched in the DOM by a **`SectionBridge`** (`components/dom/SectionBridge.tsx`, rendered between sections in `HomePage.tsx`): a zero-height thread centered on the exact room boundary announcing the next section's kicker — keep its `num`/kicker in sync when renumbering sections.
